...
public class OtpParser {
    ...
    protected static OtpErlangObject doParse(final String s)
            throws OtpParserException {
        if (s == null || s.isEmpty()) {
            return null;
        }
        return parse(scanner.scan(s));
    }
    ...
    private static OtpErlangObject parseMap(final List<Token> tokens,
        ...
        if (t.kind == TokenKind.TUPLEEND) {
            ...
            final OtpErlangObject[] all = stack
    ...
    private static enum TokenKind {
        ATOM, VARIABLE, STRING, INTEGER, PLACEHOLDER, TUPLESTART, TUPLEEND, LISTSTART, LISTEND, COMMA, CONS, MAP, ARROW, ASSIGN, UNKNOWN;
    }

    private static class Scanner {
        public Token nextToken(final String s) {
        ...
        private void scanPlaceholder(final String s, final Token result) {
        ...
        private void scanInteger(final String s, final Token result) {
            char c;
            c = s.charAt(result.end);
        ...
        private void scanVariable(final String s, final Token result) {
            char c;
            c = s.charAt(result.end);
        ...
        private void scanString(final String s, final Token result) {
            char c;
        ...
        private void scanQAtom(final String s, final Token result) {
            char c;
        ...
        private void scanAtom(final String s, final Token result) {
            char c;
            c = s.charAt(result.end);
            result.kind = TokenKind.ATOM;
            while (result.end < s.length() && c >= 'a' && c <= 'z' || c >= 'A'
        ...
        public List<Token> scan(final String s) {
            String ss = s + " ";
            final List<Token> result = new ArrayList<Token>();
            Token t = nextToken(ss);
            while (t != null) {
                result.add(t);
    ...
    private static String unescape(final String message) {
        final StreamTokenizer parser = new StreamTokenizer(new StringReader(
                message));
        String result;
        try {
            parser.nextToken();
