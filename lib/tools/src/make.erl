...
-define(MakeOpts,[noexec,load,netload,noload,return_details]).
...
do_make_files(Fs, Opts) ->
    process(Fs, lists:member(noexec, Opts), lists:member(return_details, Opts), load_opt(Opts), []).
...
process([{[],_Opts}|Rest], NoExec, Details, Load, Result) ->
    process(Rest, NoExec, Details, Load, Result);
process([{[H|T],Opts}|Rest], NoExec, Details, Load, Result) ->
    case recompilep(coerce_2_list(H), NoExec, Load, Opts) of
	error ->
	    process([{T,Opts}|Rest], NoExec, Details, Load, [{H,error}|Result]);
	Info ->
	    process([{T,Opts}|Rest], NoExec, Details, Load, [{H,Info}|Result])
    end;
process([], NoExec, Details, _Load, Result) ->
    if not NoExec ->
